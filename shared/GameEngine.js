...
export class GameEngine {
  ...
  attack(attacker, defender, field) {
    ...
    if (combatResult.winner === "attacker") {
      ...
      const penalty = GameRules.calculateMoralePenalty(
        result.defender.initialCount,
      );
  ...
  addMoraleForAll(amount, partyId) {
    const updates = GameRules.calculateGlobalMoraleUpdates(
      this.gameModel.armies,
      partyId,
      amount,
    );
  ...
  spawnUnits(partyId) {
    ...
    const ucount = Math.floor(
      (landCount + portCount * 5) / (towns.length || 1),
    );

    // Capital spawn
    const capitalField = this.gameModel.getField(
      party.capital.fx,
      party.capital.fy,
    );
    if (capitalField && capitalField.party === partyId) {
      const event = this.addUnitsToField(
        capitalField,
        5,
        party.morale,
        partyId,
  ...
  addUnitsToField(field, count, morale, partyId) {
    ...
    if (field.army) {
      ...
      const newMorale = Math.floor(
        (field.army.count * field.army.morale + count * morale) / newCount,
      );
      field.army.count =
        newCount > Config.UNITS.MAX_COUNT ? Config.UNITS.MAX_COUNT : newCount;
      field.army.morale = newMorale;
      if (field.army.morale > field.army.count)
        field.army.morale = field.army.count;
  ...
  syncPartyArmies() {
    ...
    for (const key of toDelete) {
      const army = this.gameModel.armies[key];
      if (army && army.field && army.field.army === army) {
  ...
  transferDeadFactionLands() {
    ...
    for (const key in this.gameModel.fields) {
      ...
      if (field.party >= 0) {
        ...
        while (
          landOwner.status === 0 &&
          landOwner.capital.party !== landOwner.id
        ) {
          const nextOwnerId = landOwner.capital.party;
          // Prevent infinite loops
          if (nextOwnerId === landOwner.id || nextOwnerId === field.party)
            break;
      ...
      if (field.army) {
        ...
        if (this.gameModel.parties[armyPartyId].status === 0) {
          const disbandedArmy = field.army;
          disbandedArmy.remove = true;
          field.army = null;
          delete this.gameModel.armies[disbandedArmy.id];

          events.push({
            type: "army_disbanded",
            armyId: disbandedArmy.id,
            party: armyPartyId,
            field: { fx: field.fx, fy: field.fy },
          });
        }
