...
export class Game {
  ...
  prepareImages() {
    ...
    for (let i = 1; i <= cfg.GRASS_BG.count; i++) {
      images["grassBg" + i] = {
        img: null,
        path: Utils.getImagePath(cfg.GRASS_BG.prefix + i + ".png"),
        status: "none",
      };
    }
    for (let i = 1; i <= cfg.SEA_BG.count; i++) {
      images["seaBg" + i] = {
        img: null,
        path: Utils.getImagePath(cfg.SEA_BG.prefix + i + ".png"),
        status: "none",
      };
    }
    for (let i = 1; i <= cfg.TOWN_BG_GRASS.count; i++) {
      images["townBgGrass" + i] = {
        img: null,
        path: Utils.getImagePath(cfg.TOWN_BG_GRASS.prefix + i + ".png"),
        status: "none",
      };
    }
    ...
    images["city"] = {
      img: null,
      path: Utils.getImagePath(cfg.CITY),
      status: "none",
    };
    images["port"] = {
      img: null,
      path: Utils.getImagePath(cfg.PORT),
      status: "none",
    };
  ...
  loadImage(ref) {
    return new Promise((resolve) => {
      ref.img = new Image();
      ref.img.onload = () => {
        ref.status = "Image loaded";
        resolve();
      };
      ref.img.onerror = () => {
        ref.status = "Failed to load image";
        resolve();
      };
  ...
  initGameSession(mapNumber, isSinglePlayer = true) {
    ...
    this.state.pixelWidth = Math.ceil(
      (this.state.width - 1) * (this.state.hexWidth * 0.75) +
        this.state.hexWidth,
    );
    this.state.pixelHeight =
      (this.state.height - 1) * this.state.hexHeight +
      this.state.hexHeight +
      this.state.hexHeight / 2;
    ...
    this.logic.onTerritoryChanged = () => this.mapRender.markTerritoryDirty();
    return random;
  }
  ...
  startGameFromServerData(mapData) {
    ...
    this.mapGenerator = new MapGeneratorClient(
      this.state,
      random,
      this.pathfinder,
    );
    ...
    if (mapData.parties) {
      for (let i = 0; i < mapData.parties.length; i++) {
        ...
        if (partyData.capital) {
          const capitalField = this.state.getField(
            partyData.capital.fx,
            partyData.capital.fy,
          );
  ...
  nextTurn() {
    ...
    if (this.state.humanPlayerId >= 0 && !this.state.isSpectating) {
      ...
      if (
        humanParty.provincesCp &&
        humanParty.provincesCp.length === this.state.parties.length - 1
      ) {
  ...
  runComputerTurn(partyId) {
    ...
    TurnExecutor.executeAITurn({
      ...
      getDelay: (isAnimating) => {
        return isAnimating
          ? Config.ANIMATION.MOVE_WAIT
          : Config.ANIMATION.MOVE_WAIT_MIN;
      },
      checkAnimating: () => {
  ...
  updateMapStatus() {
    ...
    if (
      this.state.turnParty >= 0 &&
      this.state.turnParty < this.state.parties.length
    ) {
  ...
  drawGame() {
    ...
    if (this.selectedArmy) {
      ...
      const possibleMoves = this.pathfinder.getPossibleMoves(
        this.selectedArmy.field,
        true,
        false,
      );
  ...
  handleInput(event) {
    ...
    if (
      this.selectedArmy &&
      this.selectedArmy.party === this.state.humanPlayerId
    ) {
      const possibleMoves = this.pathfinder.getPossibleMoves(
        this.selectedArmy.field,
        true,
  ...
  processNextAction() {
    ...
    // If queue is overloaded, skip animations and drain faster
    if (this.actionQueue.length > this.maxActionQueueSize) {
      console.warn(
        `Action queue overloaded (${this.actionQueue.length}), draining without animations`,
      );
  ...
  handleMoveExecuted(moveData) {
    ...
    this.queueAction({
      execute: () => {
        const fromField = this.state.getField(
          moveData.fromField.fx,
          moveData.fromField.fy,
        );
        const toField = this.state.getField(
          moveData.toField.fx,
          moveData.toField.fy,
        );
        ...
        if (!army && moveData.armyId) {
          ...
          if (army) {
            console.warn(
              `Army ${moveData.armyId} not at source field (${moveData.fromField.fx},${moveData.fromField.fy}), found at (${army.field.fx},${army.field.fy}). Correcting...`,
            );
        ...
        if (moveData.events) {
          // Log the move first
          const partyName = this.state.parties[moveData.playerId]
            ? this.state.parties[moveData.playerId].name
            : "Unknown";
          this.logic.updateGameLog(
            `${partyName} moved unit from (${moveData.fromField.fx},${moveData.fromField.fy}) to (${moveData.toField.fx},${moveData.toField.fy})`,
          );
          ...
          for (const event of moveData.events) {
            ...
            if (event.type === "combat") {
              ...
              combatDelay =
                ...
                Config.ANIMATION.ATTACK_RETURN_DURATION +
                0.1;
            }
          }
        }
        ...
        if (army) {
          ...
          const joinEvent = moveData.events
            ? moveData.events.find((e) => e.type === "join")
            : null;
          const hasRemainder =
            joinEvent &&
            joinEvent.movingArmy &&
            joinEvent.movingArmy.remainder > 0;
        ...
        if (
          this.isMultiplayer &&
          this.state.turnParty === this.state.humanPlayerId
        ) {
  ...
  applyServerEvent(event) {
    if (event.type === "combat") {
      ...
      if (winner && event.attacker && event.attacker.id === event.winner) {
        ...
        winner &&
        event.defender &&
        event.defender.id === event.winner
      ) {
  ...
  handleNewTurn(turnData) {
    ...
    this.queueAction({
      execute: () => {
        ...
        console.log(
          `New turn: ${turnData.turn + 1}, Party: ${turnData.partyName}, Control: ${turnData.control}`,
        );
        this.logic.updateGameLog(
          `<b>Turn ${turnData.turn + 1}: ${turnData.partyName}</b>`,
        );
        ...
        if (this.isMultiplayer && turnData.stateHash !== undefined) {
          ...
          if (clientHash !== turnData.stateHash) {
            console.error(
              `[DESYNC DETECTED] Client hash: ${clientHash}, Server hash: ${turnData.stateHash} at turn ${turnData.turn + 1}`,
            );
            ...
            console.log(
              `[Sync OK] Turn ${turnData.turn + 1} hash: ${clientHash}`,
            );
          }
        }
        ...
        if (this.isMultiplayer) {
          if (
            ...
            console.log(
              `Waiting for ${turnData.partyName} (${turnData.control}) to play`,
            );
          }
        } else {
          if (currentParty.control === "computer") {
  ...
  handleFullResync(serverState) {
    ...
    // Recalculate move budget if it's the human player's turn
    if (
      this.isMultiplayer &&
      this.state.turnParty === this.state.humanPlayerId
    ) {
      const currentParty = this.state.parties[this.state.turnParty];
      if (currentParty && currentParty.status !== 0) {
        if (
          serverState.maxMovesThisTurn !== undefined &&
          serverState.movesUsedThisTurn !== undefined
        ) {
          this.totalMovesForTurn = serverState.maxMovesThisTurn;
          this.humanMovesLeft = Math.max(
            0,
            serverState.maxMovesThisTurn - serverState.movesUsedThisTurn,
          );
        } else {
          // Fallback: infer from army moved flags
          this.totalMovesForTurn = this.getMovePoints(currentParty.id);
          const movedCount = Object.values(this.state.armies).filter(
            (a) => a.party === this.state.humanPlayerId && a.moved,
          ).length;
          this.humanMovesLeft = Math.max(
            0,
            this.totalMovesForTurn - movedCount,
          );
        }
        this.endTurnSent = false;
        this.selectedArmy = null;
        ...
        console.log(
          `[RESYNC] Restored move budget: ${this.humanMovesLeft}/${this.totalMovesForTurn}`,
        );
    ...
    // Update board
    this.mapRender.markTerritoryDirty();
    this.logic.updateBoard();
    this.updateMapStatus();
    this.updateTopBar();
    this.drawGame();
  ...
  showGameEndModal(type, winnerName) {
    ...
    if (type === "victory") {
      ...
      message.textContent =
        "You have conquered all enemy capitals and achieved total domination!";
      ...
      message.textContent =
        "Your empire has been defeated. What would you like to do?";
