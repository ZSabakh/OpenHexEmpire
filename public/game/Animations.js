...
export class Animations {
  ...
  static animateMerge(movingArmy, targetArmy) {
    ...
    if (movingArmy && !movingArmy.visual)
      movingArmy.visual = { x: movingArmy.field._x, y: movingArmy.field._y };
    if (!targetArmy.visual)
      targetArmy.visual = { x: targetArmy.field._x, y: targetArmy.field._y };
    ...
    if (movingArmy) {
      ...
      tl.to(
        [movingArmy.visual, targetArmy.visual],
        {
  ...
  static animateAttack(attacker, defender) {
    ...
    tl.to(
      defender.visual,
      {
        x:
          defenderOrigX +
          (dx / distance) * Config.ANIMATION.ATTACK_RECOIL_DISTANCE,
        y:
          defenderOrigY +
          (dy / distance) * Config.ANIMATION.ATTACK_RECOIL_DISTANCE,
        scale: 0.9,
    ...
    tl.to(
      [attacker.visual, defender.visual],
      {
        opacity: 0.6,
    ...
    tl.to(
      ...
      Config.ANIMATION.ATTACK_LUNGE_DURATION +
        Config.ANIMATION.ATTACK_IMPACT_DURATION * 0.3,
    );
  ...
  static animateMove(army, targetX, targetY, delay = 0) {
    ...
    // Kill any existing tweens on this visual to prevent competing animations
    gsap.killTweensOf(army.visual);
