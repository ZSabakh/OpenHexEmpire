...
export class MapRender {
    ...
    for (let x = 0; x < state.width; x++) {
      for (let y = 0; y < state.height; y++) {
        ...
        if (
          field.army &&
          !field.army.moved &&
          field.army.party === state.humanPlayerId
        ) {
  ...
  drawEstateIcons(ctx, state) {
    ...
    for (const field of estates) {
      ...
      if (field.estate === "town") {
        ...
        const cityImg = isCapital
          ? this.images["capital" + field.capital].img
          : this.images.city.img;
        const w = 32;
        const h = 32;
    ...
    for (let x = 0; x < state.width; x++) {
      for (let y = 0; y < state.height; y++) {
        ...
        if (field.estate === "town") {
          ...
          const bgImg = this.images["townBgGrass" + idx]
            ? this.images["townBgGrass" + idx].img
            : null;
  ...
  drawArmyUnit(ctx, army, state) {
    ...
    const scale =
      army.visual && army.visual.scale !== undefined ? army.visual.scale : 1;
    const rotation =
      army.visual && army.visual.rotation !== undefined
        ? army.visual.rotation
        : 0;
    const opacity =
      army.visual && army.visual.opacity !== undefined
        ? army.visual.opacity
        : 1;
  ...
  drawArmyLabel(ctx, army, cursorPos, isGameRunning) {
    ...
    const darkenedRgb = rgbValues
      .map((v) => Math.floor(v * mixFactor))
      .join(",");
