...
export class GameRoom {
  ...
  initializeGame() {
    ...
    console.log(
      `[GameRoom ${this.roomId}] Map generated with seed ${this.mapSeed}`,
    );
  }
  ...
  addPlayer(socketId, playerName) {
    ...
    console.log(
      `[GameRoom ${this.roomId}] Player ${playerName} joined (faction not yet chosen)`,
    );
  ...
  setFactionSelection(partyId, playerName, socketId) {
    ...
    if (
      this.factionSelections[partyId] &&
      this.factionSelections[partyId].socketId !== socketId
    ) {
    ...
    if (this.gameModel.parties[partyId]) {
      ...
      console.log(
        `[GameRoom ${this.roomId}] Faction ${partyId} set to human control for ${playerName}`,
      );
    }

    console.log(
      `[GameRoom ${this.roomId}] Faction ${partyId} selected by ${playerName}`,
    );
    return { success: true };
  }
  ...
  setPlayerReady(socketId, isReady) {
    ...
    if (player && player.partyId !== null) {
      ...
      console.log(
        `[GameRoom ${this.roomId}] Player ${player.name} (faction ${player.partyId}) ready status: ${isReady}`,
      );
      return true;
    }
    return false;
  ...
  getReadyCount() {
    return Object.values(this.readyStatus).filter((status) => status === true)
      .length;
  }
    ...
    if (player) {
      if (player.partyId !== null) {
        ...
        if (this.gameStarted && this.gameModel.parties[player.partyId]) {
          this.gameModel.parties[player.partyId].control = "computer";
          console.log(
            `[GameRoom ${this.roomId}] Converted faction ${player.partyId} (${this.gameModel.parties[player.partyId].name}) to AI control`,
          );
        } else {
          console.log(
            `[GameRoom ${this.roomId}] Cleared faction ${player.partyId} for leaving player`,
          );
        }
      }
  ...
  getGameState() {
    return {
      ...
      movesUsedThisTurn: this.movesUsedThisTurn,
      maxMovesThisTurn: this.maxMovesThisTurn,
    };
  }
  ...
  serializeParties() {
    return this.gameModel.parties.map((party) => ({
      ...
      capital: party.capital
        ? { fx: party.capital.fx, fy: party.capital.fy }
        : null,
      morale: party.morale,
      status: party.status,
  ...
  startGame() {
    ...
    console.log(
      `[GameRoom ${this.roomId}] Game started with ${this.players.size} players`,
    );
  ...
  nextTurn() {
    ...
    if (activeParties.length === 1 && this.players.size > 0) {
      console.log(
        `[GameRoom ${this.roomId}] Game Won by ${activeParties[0].name}`,
      );
      return {
        gameEnded: true,
        reason: "victory",
    ...
    console.log(
      `[GameRoom ${this.roomId}] Turn ${this.gameModel.turn + 1}, Party ${this.gameModel.turnParty} (${currentParty.name}) - ${currentParty.control}`,
    );
  ...
  validateMove(socketId, moveData) {
    ...
    const fromField = this.gameModel.getField(
      moveData.fromField.fx,
      moveData.fromField.fy,
    );
    const toField = this.gameModel.getField(
      moveData.toField.fx,
      moveData.toField.fy,
    );
    ...
    if (!possibleMoves.includes(toField)) {
      return {
        valid: false,
        error: "Target field is not reachable from source",
      };
    }
